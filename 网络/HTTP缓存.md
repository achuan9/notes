- [新鲜度](#%e6%96%b0%e9%b2%9c%e5%ba%a6)
  - [缓存寿命](#%e7%bc%93%e5%ad%98%e5%af%bf%e5%91%bd)
  - [缓存驱逐](#%e7%bc%93%e5%ad%98%e9%a9%b1%e9%80%90)
...
- [重定向响应状态码](#%e9%87%8d%e5%ae%9a%e5%90%91%e5%93%8d%e5%ba%94%e7%8a%b6%e6%80%81%e7%a0%81)
...
当服务器上面的资源进行了更新，那么缓存中的对应资源也应该被更新，由于 HTTP 是 C/S 模式的协议，服务器更新一个资源时，不可能直接通知客户端更新缓存，所以双方必须为该资源约定一个过期时间，在该过期时间之前，该资源（缓存副本）就是新鲜的，当过了过期时间后，该资源（缓存副本）则变为陈旧的。

## 缓存寿命

浏览器会按照下面的顺序来判断缓存副本是否为陈旧的：

1. 是否含有 Cache-control: max-age=N ，缓存的寿命就是 N。
2. 是否含有 Expires 属性。如果有则比较 Expires 的值和 Date 属性的值来判断是否缓存还有效。
3. 是否含有 Last-Modified 属性。如果有，缓存的寿命就等于 Date 的值减去 Last-Modified 的值除以 10（注：根据 rfc2626 其实也就是乘以 10%）。

## 缓存驱逐

由于缓存只有有限的空间用于存储资源副本，所以缓存会定期地将一些副本删除，这个过程叫做缓存驱逐。驱逐算法用于将陈旧的资源（缓存副本），也就是寿命已尽的缓存副本，替换为新鲜的，注意，一个陈旧的资源（缓存副本）是不会直接被清除或忽略的。

客户端发起一个请求时，缓存检索到已有一个对应的陈旧资源（缓存副本），则缓存会先将此请求附加一个请求头来判断是否新鲜。优先级从高到低：

- `If-None-Match`请求头，如果服务器找到对应的资源（`ETag` 值等于 `If-None-Match` 的值），则返回`304 (Not Modified)`。需要注意的是，服务器端在生成状态码为 304 的响应的时候，必须同时生成以下会存在于对应的 200 响应中的首部：Cache-Control、Content-Location、Date、ETag、Expires 和 Vary 。

- `If-Modified-Since`请求头，值为 `Last-Modified` 响应头的值，如果请求的资源从那时起未经修改，则返回，`304 (Not Modified)`。一般用来更新没有特定 ETag 标签的缓存实体。只能应用于 GET 或 HEAD 请求。
...
Cache-Control 头是 HTTP/1.1 定义的，通过它提供的不同的值来定义缓存策略，请求头和响应头都支持这个属性。

- **Cache-Control: no-store**  
  禁止进行缓存。每次由客户端发起的请求都会下载完整的响应内容。

- **Cache-Control: no-cache**  
  强制确认缓存。每次有请求发出时，缓存会将此请求发到服务器（译者注：该请求应该会带有与本地缓存相关的验证字段），服务器端会验证请求中所描述的缓存是否过期，若未过期（注：实际就是返回 304），则缓存才使用本地缓存副本。

- **Cache-Control: private**  
  表示该响应是专用于某单个用户的，中间人不能缓存此响应，该响应只能应用于浏览器私有缓存中。

- **Cache-Control: public**  
  "public" 指令表示该响应可以被任何中间人（比如中间代理、CDN 等）缓存。若指定了"public"，则一些通常不被中间人缓存的页面（因为默认是 private）（比如 带有 HTTP 验证信息（帐号密码）的页面 或 某些特定状态码的页面），将会被其缓存。
...
Expires 响应头包含日期/时间， 即在此时候之后，响应过期。无效的日期，比如 0, 代表着过去的日期，即该资源已经过期。如果在 Cache-Control 响应头设置了 "max-age" 或者 "s-max-age" 指令，那么 Expires 头会被忽略。
...
表明请求的资源被暂时的移动到了由 Location 头部指定的 URL 上。浏览器会重定向到这个 URL， 但是搜索引擎不会对该资源的链接进行更新 (In SEO-speak, it is said that the link-juice is not sent to the new URL)。
...
原始请求中的请求方法和消息主体会在重定向请求中被重用。在确实需要将重定向请求的方法转换为 GET 的场景下，可以考虑使用 303 See Also 状态码。例如在使用 PUT 方法进行文件上传操作时，需要返回确认信息（例如“你已经成功上传了 xyz”）而不是上传的资源本身，就可以使用这个状态码。

...
export default {
  name: 'swipeout-item',
  props: {
    sensitivity: { //滑动多少距离后开始触发菜单显示
      type: Number,
      default: 0
    },
    autoCloseOnButtonClick: { //点击按钮后是否收回菜单
      type: Boolean,
      default: true
    },
    disabled: Boolean, //是否不可滑动
    threshold: { //滑动多少距离后自动打开菜单，否则收回。可以为小于1的比例或者宽度值
      type: Number,
      default: 0.3
    },
    underlayColor: String,
    transitionMode: {//菜单打开方式，reveal表示菜单不动内容滑出，follow表示菜单随内容滑出
      type: String,
      default: 'reveal'
    }
  },
  ...
  methods: {
    ...
    caculateMenuWidth (direction) {
      ...
      let width = 0
      list.forEach(one => {
        //shengchuang修改的地方，自动匹配按钮宽度
        width += window.getComputedStyle(one.elm).width.replace('px', '')*1;
